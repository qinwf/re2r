...
test_that("check rewrite", {
    ...
    expect_identical(re2_extract("boris@kremvax.ru", "(.*)@([^.]*)","\\2!\\1", parallel = T),"kremvax!boris")
    expect_identical(re2_extract("foo", ".*", "'\\0'"),"'foo'")
    expect_identical(re2_extract("foo", ".*", "'\\0'", parallel = T),"'foo'")
    expect_true(is.na(re2_extract("baz", "bar", "'\\0'")))
    expect_true(is.na(re2_extract("baz", "bar", "'\\0'", parallel = T)))
